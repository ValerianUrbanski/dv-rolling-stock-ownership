...
public class Equipment
{
	...
	public string? CarGuidCoupledFront
	{
		get
		{
			//Main.LogDebug(() => $"Getting front coupling for {ID} / {CarGUID} (currently: {_carGuidCoupledFront})");
			return _carGuidCoupledFront;
		}
		private set
		{
			Main.LogDebug(() => $"Setting front coupling for {ID} / {CarGUID} to {value}.");
	...
	public string? CarGuidCoupledRear
	{
		get
		{
			//Main.LogDebug(() => $"Getting rear coupling for {ID} / {CarGUID} (currently: {_carGuidCoupledRear})");
			return _carGuidCoupledRear;
		}
		private set
		{
			Main.LogDebug(() => $"Setting rear coupling for {ID} / {CarGUID} to {value}.");
	...
	{
		Main.Log($"Creating equipment record from train car {trainCar.ID}.");
	...
	{
		Main.Log($"Updating equipment record for train car {trainCar.ID}{(isBeingDespawned ? ", which is being despawned" : "")}.");
		var bogie1 = trainCar.Bogies[0];
		var bogie2 = trainCar.Bogies[1];
		var carState = trainCar.GetComponent<CarStateSave>();
		var simCarState = trainCar.GetComponent<SimCarStateSave>();
	...
	private void SetupCouplerEventHandlers(TrainCar? trainCar)
	{
		...
		Main.LogDebug(() => $"Setting up coupler event handlers for {ID}.");
	...
	private void RemoveCouplerEventHandlers(TrainCar? trainCar)
	{
		...
		Main.LogDebug(() => $"Removing coupler event handlers for {ID}.");
	...
	public bool PrepareForDespawning()
	{
		if (!IsSpawned)
		{
			Main.LogError($"Attempting to prepare equipment with ID {ID} for despawning, but it is not currently known to be spawned!");
			return false;
		}

		Main.Log($"Preparing equipment record with ID {ID} for despawning.");
	...
	public TrainCar Spawn()
	{
		if (IsSpawned)
		{
			Main.LogWarning($"Trying to spawn train car based on equipment record with ID {ID}, but it already exists!");
	...
	public TrainCar? GetTrainCar()
	{
		if (!IsSpawned)
		{
			Main.Log($"Trying to get the logic car of equipment record with ID {ID}, but it isn't spawned!");
			return null;
		}
	...
	public bool IsRecordOf(TrainCar? trainCar)
	{
		if (trainCar == null)
		{
			Main.LogWarning("Train car is null. Can't compare equipment record. Returning false.");
			return false;
		}
		if (trainCar.logicCar != null) { return CarGUID == trainCar.CarGUID; }
		if (IsSpawned) { return this.trainCar == trainCar; }
		Main.LogWarning($"Trying to compare an unspawned equipment record with ID {ID} and a train car without a logic car. Returning false, but this may have unexpected side effects.");
		return false;
	}
	...
	public static Equipment? FromSaveData(JObject data)
	{
		...
		Main.Log($"Restoring equipment record with ID {id} and spawn state is {(isSpawned ? "spawned" : "not spawned")} from save data.");
		...
		if (isSpawned)
		{
			...
			if (trainCar == null)
			{
				Main.LogWarning($"Couldn't find train car for spawned equipment with ID {id}. Marking as not spawned.");
				isSpawned = false;
			}
		}
